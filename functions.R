...
oliveID_rm.ppm_fn <- function(input_rds, save_rdata){
  ...
  for(i in 1:length(input_list)){
    ...
    output$class_ID <- as.factor(output$class_ID)
...
name_relist_fn <- function(input_rds, data_class){
  ...
  for(i in 1:length(input_list)){
    ...
    root_name <- names(input_list[i])
    
    bin60_name <- paste0(root_name, "_", data_class, "_bin60.rds")
    bin400_name <- paste0(root_name, "_", data_class, "_bin400.rds")
    nat60_pbin400_name <- paste0(root_name, "_", data_class, "_nat60_pbin400.rds")
    zf60_name <- paste0(root_name, "_", data_class, "_zf60.rds")
    nat400_name <- paste0(root_name, "_", data_class, "_nat400.rds")
    zf400_name <- paste0(root_name, "_", data_class, "_zf0.rds")
    
    saveRDS(input_list[[i]][1:4], file = bin60_name)
    saveRDS(input_list[[i]][5:6], file = bin400_name)
    saveRDS(input_list[[i]][7:12], file = nat60_pbin400_name)
    saveRDS(input_list[[i]][13:16], file = zf60_name)
    saveRDS(input_list[[i]][17:18], file = nat400_name)
    saveRDS(input_list[[i]][19:20], file = zf400_name)
    
    output_list[[i]] <- list(bin60_name, bin400_name, nat60_pbin400_name,
                             zf60_name, nat400_name, zf400_name)
  }
...
rf_train_fn <- function(input_names_rds, load_rdata, control){
  
  for(j in 1:length(input_names_rds)){
    
    input_list <- readRDS(input_names_rds[j])
    load(load_rdata)
    ...
    for(i in 1:length(input_list)){
      ...
      data <- get(input_list[[i]])[,-c(1,2)]
      
      data$class_ID <- as.factor(data$class_ID)
      
      input_name <- gsub("(.*)_train\\.\\d(_as|_ps|_snv)", "\\1\\2", input_list[[i]])
      partition_seed <-  gsub(".*_train\\.(\\d)(_as|_ps|_snv)", "\\1", input_list[[i]])
      ...
      output_name <- paste0(input_name, "_rfmodel.", partition_seed)
      output_rdata <- paste0(input_name, "_rfmodel.", partition_seed, ".RData")
      
      assign(output_name, model, envir = globalenv())
      save(list = output_name, file = output_rdata)
...
plsda_train_fn <- function(input_names_rds, load_rdata, control){
  ...
  for(j in 1:length(input_names_rds)){
    ...
    for(i in 1:length(input_list)){
      ...
      output_name <- paste0(input_name, "_plsmodel.", partition_seed)
      output_rdata <- paste0(input_name, "_plsmodel.", partition_seed, ".RData")
...
#### call_svm_linear_train_fn ####

call_svm_linear_train_fn <- function(input_data, control, grid){
...
#### call_svm_radial_train_fn ####

call_svm_radial_train_fn <- function(input_data, control, custom_TL){
...
#### call_svm_poly_train_fn ####

call_svm_poly_train_fn <- function(input_data, control, custom_TL){
...
#### evaluate_svm_triple_fn ####

evaluate_svm_triple_fn <- function(input_names_rds, load_rdata, control){
  
  load(load_rdata)
  fitControl <- control
  output_list <- list()
  
  for(i in 1:length(input_names_rds)){
    
    input_list <- readRDS(input_names_rds[i])
    ...
    for(j in 1:length(input_list)){
      ...
      unregister_dopar
      data <- get(input_list[[j]])[,-c(1,2)]
      
      linear_model <- call_svm_linear_train_fn(input_data = data,
                                               control = fitControl)
      unregister_dopar
      radial_model <- call_svm_radial_train_fn(input_data = data,
                                               control = fitControl) 
      unregister_dopar
      poly_model <- call_svm_poly_train_fn(input_data = data,
                                           control = fitControl) 
      unregister_dopar
      print(list("lm_besttune" = linear_model$bestTune,
                 "rm_besttune" = radial_model$bestTune,
                 "pm_besttune" = poly_model$bestTune))
      
      output_list_i[[j]] <- list(linear_model$results[which.max(linear_model$results[,3]),],
                                  radial_model$results[which.max(radial_model$results[,4]),],
                                  poly_model$results[which.max(poly_model$results[,5]),])
      names(output_list_i[[j]]) <- c("linear", "radial", "poly")
    }
    
    names(output_list_i) <- gsub("(.*)_train\\.\\d(_as|_ps|_snv)", "\\1\\2", input_list)
    output_list[[i]] <- output_list_i
  }
